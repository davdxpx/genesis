...
// Base stats depending on the Embryo chosen in Phase 6
const baseEmbryoStats: Record<string, Record<string, number>> = {
  'alpha-1': { int: 130, phy: 70, imm: 80, life: 80 }, // Genie
  'beta-4':  { int: 85, phy: 130, imm: 90, life: 60 }, // Athlet
  'gamma-9': { int: 90, phy: 90, imm: 130, life: 90 }, // Resilient
  'delta-2': { int: 110, phy: 75, imm: 70, life: 75 }, // Kreativ
...
export function BabyDesignerStatsPhase({ onNext, gameState }: { onNext: () => void, gameState: GameStateProps }) {
  ...
  const initialAvailablePoints = Math.max(5, 15 + budgetBonus); // Minimum 5 points
  ...
  const handleSub = (statId: string) => {
    // Cannot go below base embryo stats!
  ...
  // Center is 150,150. Max radius is 120. Scale: max stat 200 = 120px radius.
  ...
  return (
    ...
    >
      ...
      {/* ==========================================
          RIGHT PANE: IPAD-FRIENDLY CONTROLS & LORE
      ========================================== */}
      <div className="flex-1 flex flex-col gap-4 h-full relative z-20">
         ...
         <Card className="flex-1 glass border-[#00f0ff]/20 flex flex-col relative overflow-hidden">
            ...
            <CardContent className="flex-1 overflow-y-auto custom-scrollbar p-3 md:p-6 space-y-4">
               {/* Massive, touch-friendly stat controls */}
